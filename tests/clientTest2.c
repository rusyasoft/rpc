...
int main(int arc, char ** argv){
    ...
    rpc_add_procedure(test_rpc, &test_string_concat_STRING_2args);



	uint8_t uint8_a = 10, uint8_b = 20;
	int * retstr = (int*)rpcclt_invoke(test_rpc, "sum_UINT8_2args", uint8_a, uint8_b);
	if (retstr != NULL)
		printf("clientTest2: sum_UINT8_2args, return = %d\n",  (uint16_t)*retstr);

	uint16_t uint16_a = 10, uint16_b = 20;
	retstr = (int*)rpcclt_invoke(test_rpc, "sum_UINT16_2args", uint16_a, uint16_b);
	if (retstr != NULL)
		printf("clientTest2: sum_UINT16_2args, return = %d\n", *retstr);
	
	uint32_t uint32_a = 10, uint32_b = 20;
	retstr = (int*)rpcclt_invoke(test_rpc, "sum_UINT32_2args", uint32_a, uint32_b);
	if (retstr != NULL)
		printf("clientTest2: sum_UINT32_2args, return = %d\n", *retstr);

	uint64_t uint64_a = 10, uint64_b = 20;
	retstr = (int*)rpcclt_invoke(test_rpc, "sum_UINT64_2args", uint64_a, uint64_b);
	if (retstr != NULL)
		printf("clientTest2: sum_UINT64_2args, return = %d\n", *retstr);

	float float_a = 11.1, float_b = 22.2;
    printf("clientTest2: float_a = %f, float_b = %f\n", float_a, float_b);
	float * float_retstr = (float*)rpcclt_invoke(test_rpc, "sum_FLOAT_2args", float_a, float_b);
	if (float_retstr != NULL)
		printf("clientTest2: sum_FLOAT_2args, return = %f\n", *float_retstr);

	double double_a = 10.1, double_b = 20.1;
    printf("clientTest2: float_a = %f, float_b = %f\n", double_a, double_b);
	double * double_retstr = (double*)rpcclt_invoke(test_rpc, "sum_DOUBLE_2args", double_a, double_b);
    ...
    string * s_ret = (int*)rpcclt_invoke(test_rpc, "string_concat", str_a, str_b);
    if (s_ret != NULL)
